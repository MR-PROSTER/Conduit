    ...
    router.post("/me/profile", async (req: Request, res: Response) => {
        try {
            ...
            const {
                ...
                error: authError,
            } = await supabase.auth.admin.updateUserById(user.id, { user_metadata: { name: cleanName } });
  ...
  <script>
    ...
    router.get("/auth/exchange", async (req: Request, res: Response) => {
        try {
            ...
            res.redirect(
                `http://localhost:${cleanPort}/callback?accessToken=${data.session.access_token}&user=${userStr}`,
            );
        } catch (error) {
            sendError(res, error);
        }
